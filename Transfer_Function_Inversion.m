...
use_tikhonov          = true;     % if true: conj(H)./(|H|^2+lambda) regularization
tikhonov_lambda       = 1e-6;      % base Tikhonov level lambda0
...
f_iri_min             = 0.2;       % lower edge of IRI-relevant band [Hz]
f_iri_max             = 60;        % upper edge of IRI-relevant band [Hz]
lambda_iri_factor     = 1e-3;      % lambda multiplier inside IRI band (milder reg.)
lambda_outband_factor = 2;         % lambda multiplier outside IRI band (stronger reg.)
...
% Define usable frequency band
f_max = min(f_max_cap, 0.95*(fs/2));          % Hz

band_mask = (freq >= f_min) & (freq <= f_max);

if use_tikhonov
    fprintf('Inversion: band-limited *frequency-dependent* Tikhonov on [%.2f, %.2f] Hz\n', ...
        f_min, f_max);
    ...
    % Base level
    lambda0    = tikhonov_lambda;

    % IRI-relevant band (tune via hyperparameters)
    f_iri_min_eff  = max(f_iri_min, f_min);      % Hz
    f_iri_max_eff  = min(f_iri_max, f_max);      % Hz

    iri_band   = (freq >= f_iri_min_eff) & (freq <= f_iri_max_eff);

    % Build lambda(f):
    %   - very mild regularization inside IRI band
    %   - stronger regularization outside (still within [f_min, f_max])
    lambda_vec                      = lambda0 * ones(size(freq));
    lambda_vec(iri_band)            = lambda0 * lambda_iri_factor;
    lambda_vec(band_mask & ~iri_band) = lambda0 * lambda_outband_factor;

    % Tikhonov denominator
    reg = Hmag2 + lambda_vec;

    % Apply inversion only in band_mask
    idx = band_mask;
    Road_f(idx) = (conj(Hf(idx)) ./ reg(idx)) .* Accel_f(idx);

    valid = idx;

    % Optional: report average shrinkage in IRI band (diagnostic)
    if any(iri_band)
        alpha = Hmag2 ./ max(reg, eps);           % shrinkage factor
        mean_alpha_iri = mean(alpha(iri_band));
        fprintf('  Mean shrinkage in IRI band [%.2f, %.2f] Hz: %.3f\n', ...
            f_iri_min_eff, f_iri_max_eff, mean_alpha_iri);
    end

else
    fprintf('Inversion: band-limited hard threshold |H(f)| > %.3e on [%.2f, %.2f] Hz\n', ...
        inversion_threshold, f_min, f_max);

    Road_f = zeros(size(Accel_f));
    valid  = band_mask & (abs(Hf) > inversion_threshold);
    Road_f(valid) = Accel_f(valid) ./ Hf(valid);
end

%% --------------------- 4) Back to time, then to spatial ------------------
zr_t = ifft(Road_f, 'symmetric');
zr_t = zr_t(1:N);
