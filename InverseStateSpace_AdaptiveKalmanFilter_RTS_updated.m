...
opts.restore_mean_after_filter = false;
...
opts.akf.enabled     = false;            % Enable innovation-based Q scaling
...
function [Xs, Xf] = kf_rts_smoother_1d(A, C, Q, R, y, x0, P0, adapt_opts)
    ...
    for k = 1:N
        ...
        % 3. Update (Joseph Form - Numerically Stable)
        % Add stronger regularization to P_pred for large signals
        P_pred = P_pred + 1e-10 * eye(size(P_pred)); 
        
        S = C * P_pred * C.' + R;
        
        % Check for singular S before division
        if rcond(S) < 1e-12
            K = zeros(size(P_pred,1), 1); % Fallback: ignore measurement if singular
        else
            K = (P_pred * C.') / S;       % (n x 1)
        end
        ...
        % --- JOSEPH FORM UPDATE (Guarantees P remains Positive Definite) ---
        I_mat = eye(n);
        IKC = I_mat - K * C;
        % Formula: P = (I-KC)*P_pred*(I-KC)' + K*R*K'
        P = IKC * P_pred * IKC.' + K * R * K.'; 
        
        P = (P + P.') / 2;                % Final symmetry check
    ...
    for k = N-1:-1:1
        ...
        % Use pinv (Pseudo-Inverse) to safely handle singular matrices
        G = (Pk * A.') * pinv(Pkp);
