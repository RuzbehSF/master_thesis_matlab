...
%   - Slope approx +X.0 : Integration Drift (Low-frequency instability)
%
% INPUTS:
%   Auto-detects 'recon_spatial.mat' from your output folders.
%
% OUTPUTS:
%   - A diagnostic plot (PSD vs ISO Reference).
%   - A console report with the calculated slope and verdict.
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
% Search for the file in the standard path structure
file_candidates = { ...
    ...
    'recon_spatial.mat' % Current dir fallback
};
...
% Ensure column vector and remove mean
zr = zr(:);
zr = detrend(zr, 'constant'); % Remove DC offset
...
if isempty(idx_fit)
    warning('Spatial sampling rate too low to analyze road roughness band.');
    idx_fit = 2:length(n_spatial); % Fallback to all
end
...
% Anchor the reference line to the middle of the fit data
...
title(['Spectral Consistency Check: ' name]);
