...
sigma_az = getfield_with_default(cfg,'sigma_az', []);           % accel noise std [m/s^2]
if isempty(sigma_az) || ~isfinite(sigma_az) || sigma_az<=0
    sigma_az = max(1e-3, 0.05*std(azs,'omitnan'));              % fallback guess
end
R = sigma_az^2;

% Process noise per-step on states (DISCRETE); tune these.
% Bigger -> more “freedom” (less smooth, more noise). Smaller -> smoother but can lag/bias.
q_zs  = 1e-10;
q_zsd = 1e-8;
q_zu  = 1e-10;
q_zud = 1e-8;
q_zr  = 1e-7;
q_zrd = 1e-5;              % usually the biggest (lets road slope/shape adapt)
Q = diag([q_zs q_zsd q_zu q_zud q_zr q_zrd]);
...
A(5,6) = 1;
% A(6,:) = 0;   % z_rdotdot treated as process noise via Q on z_rdot (discrete)
...
x0 = zeros(6,1);
P0 = diag([1e-2 1e0 1e-2 1e0 1e-2 1e0]);   % loose initial uncertainty

[Xs, Xf] = kf_rts_smoother_1d(Ad, Cd, Q, R, y, x0, P0);
...
function [Xs, Xf] = kf_rts_smoother_1d(A, C, Q, R, y, x0, P0)
    % Discrete-time Kalman filter + RTS smoother for 1D measurement:
