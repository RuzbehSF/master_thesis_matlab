...
%%% Smartphone accelerometer emulation (phone on dashboard)
smart.enabled             = false;       % master switch
smart.include_gravity     = false;       % phones output specific force (≈ +g at rest)
smart.euler_body2dev_deg  = [0, 8, 0];  % [yaw_Z, pitch_Y, roll_X] in deg (typical dash tilt)
smart.mount_fc_hz         = [];         % dash/mount low-pass corner (Hz). [] to skip
smart.fs_hz               = 200;        % phone logging rate (Hz) e.g., 100 or 200
smart.aa_frac_nyq         = 0.8;        % anti-alias LPF at 0.8*Nyquist before downsample
smart.range_g             = 2;          % ±g range of the device (±2/±4/±8)
smart.bits                = 16;         % effective ADC resolution
...
% 'Body_Resonance_Track'         (Sinusoidal input tuned to body mode, strong low-freq excitation)
% 'Wheelhop_Resonance_Track'     (Sinusoidal input tuned to wheel-hop mode, high-freq excitation)
% 'Cobblestone_Street'           (Dense small bumps, cobblestone-like texture)
% 'Gravel_Road'                  (Very rough, loose surface with many small pits and some corrugation)
% 'Rail_Crossing'                (Localized high-impact rail/tram crossing event)
% 'Random_Damage_Field'          (Random mix of bumps/potholes/ruts/etc. over the full length)

damage_scenario_selection = 'All_Damages_Mixed_Severity';
...
% Store approximate body / wheel-hop frequencies for resonance scenarios
if numel(fu) >= 2
    f_body_mode_hz     = fu(1);
    f_wheelhop_mode_hz = fu(2);
elseif isscalar(fu)
    f_body_mode_hz     = fu(1);
    f_wheelhop_mode_hz = NaN;
else
    f_body_mode_hz     = NaN;
    f_wheelhop_mode_hz = NaN;
end
...
[spatial_road, z_r_x, time_sim, z_r_t, dz_r_t, z_r_x_base] = ...
    generateRoadProfile(Road_Length, V, dt, time_road_profile, road_class, ...
                        damage_scenario_selection, f_body_mode_hz, f_wheelhop_mode_hz);
...
%% 7b) Smartphone accelerometer channel (device-frame, gravity, mount, sampling, quantization)
smartphone = struct();   % output container

if smart.enabled
    % --- body vertical accel (clean) ---
    a_body_z = sprung_mass_accel_raw(:);  % m/s^2
    % mirror sensor noise for phone channel (independent of 'sprung_mass_accel')
    if enable_noise
        nyquist_freq = Sampling_Freq / 2;
        noise_std_g     = noise_density_g_sqrt_hz * sqrt(nyquist_freq);
        noise_std_mps2  = noise_std_g * g0;
        white_noise_p   = noise_std_mps2 * randn(N, 1);
        bias_drift_std_mps2 = bias_drift_std_g_per_step * g0;
        bias_drift_p    = cumsum(bias_drift_std_mps2 * randn(N, 1));
        bias_offset_mps2_p  = bias_offset_g * g0;
        a_body_z_noisy = a_body_z + white_noise_p + bias_drift_p + bias_offset_mps2_p;
    else
        a_body_z_noisy = a_body_z;
    end

    % phones measure specific force (≈ a + g in Z-up body/world)
    if smart.include_gravity
        f_z = a_body_z_noisy + g0;  % m/s^2, ≈ +g at rest
    else
        f_z = a_body_z_noisy;       % "linear acceleration" style
    end

    % light low-pass to emulate dash/mount
    fz_mount = f_z;
    if ~isempty(smart.mount_fc_hz) && smart.mount_fc_hz > 0 && smart.mount_fc_hz < (Sampling_Freq/2)
        [bm, am] = butter(2, smart.mount_fc_hz/(Sampling_Freq/2), 'low');
        fz_mount = filtfilt(bm, am, fz_mount);
    end

    % rotate body->device (ZYX eulers). With only vertical input, this scales by R(3,3)
    R_bd = rotmZYX(smart.euler_body2dev_deg);
    R33  = R_bd(3,3);
    acc_dev_z_fullrate = R33 * fz_mount;   % device Z-axis specific force

    % anti-alias then resample to phone rate
    Fs_sim = Sampling_Freq;
    Fs_ph  = smart.fs_hz;
    acc_f = acc_dev_z_fullrate;
    if Fs_ph < Fs_sim
        Wn = (smart.aa_frac_nyq * (Fs_ph/2)) / (Fs_sim/2);
        Wn = min(max(Wn, 1e-3), 0.99);
        [ba, aa] = butter(4, Wn, 'low');
        acc_f = filtfilt(ba, aa, acc_f);
    end
    t_phone = (0:1/Fs_ph:t_sol(end)).';
    phone_accel_unq = interp1(t_sol, acc_f, t_phone, 'linear', 'extrap');

    % quantize and saturate to ±range
    range_mps2 = smart.range_g * g0;
    q_step     = 2*range_mps2 / (2^smart.bits - 1);
    phone_accel_sat = max(min(phone_accel_unq, +range_mps2), -range_mps2);
    phone_accel_q   = q_step * round(phone_accel_sat / q_step);

    % stash
    smartphone.t               = t_phone;
    smartphone.accel_z_mps2    = phone_accel_q;
    smartphone.accel_z_unquant = phone_accel_unq;
    smartphone.settings        = smart;
    smartphone.fs_hz = Fs_ph;   % convenience copy of phone rate
else
    smartphone = [];
end

%% 8) Visualization (publication-grade dashboard)
g0 = 9.806;
...
COL.heave      = [0.00 0.45 0.74];
COL.defl       = [0.47 0.67 0.19];
COL.acc_clean  = [0.00 0.00 0.00];
COL.acc_meas   = [0.85 0.33 0.10];
COL.acc_phone  = [0.63 0.08 0.18];
COL.error      = [0.55 0.00 0.55];
COL.shade      = [0.93 0.96 1.00];

% -------------------------- Plot decimation -------------------------------
Nplot = 6000;  % max points per trace (display only)
idx_t  = unique(round(linspace(1, numel(t_sol),        min(numel(t_sol),        Nplot))));
idx_x  = unique(round(linspace(1, numel(spatial_road), min(numel(spatial_road), Nplot))));

% Derived signals for visualization
Susp_Defl_m = Sprung_Mass_Disp - Unsprung_Mass_Disp;  % (ys - yus)

% Does a smartphone struct exist with time & accel?
phone_exists = exist('smartphone','var') && isstruct(smartphone) && ~isempty(smartphone) ...
    && isfield(smartphone,'t') && isfield(smartphone,'accel_z_mps2') ...
    && numel(smartphone.t) >= 2 && numel(smartphone.accel_z_mps2) == numel(smartphone.t);

if phone_exists
    % Pull and sanitize phone stream
    t_meas = smartphone.t(:);
    a_meas = smartphone.accel_z_mps2(:);

    % Remove NaNs / non-finite
    mfin = isfinite(t_meas) & isfinite(a_meas);
    t_meas = t_meas(mfin);
    a_meas = a_meas(mfin);

    % Sort by time (in case)
    [t_meas, ord] = sort(t_meas);
    a_meas = a_meas(ord);

    % Drop duplicate timestamps
    [t_meas, uniqIdx] = unique(t_meas, 'stable');
    a_meas = a_meas(uniqIdx);

    % Use provided fs if present; otherwise infer from median dt
    if isfield(smartphone, 'fs_hz') && ~isempty(smartphone.fs_hz) && isfinite(smartphone.fs_hz)
        fs_meas = smartphone.fs_hz;
    else
        dtm = median(diff(t_meas));
        fs_meas = 1 / max(dtm, eps);
    end

    % Optional: auto-detect g-units and convert to m/s^2 (uncomment if needed)
    % if ~isfield(smartphone,'units') && max(abs(a_meas)) < 5  % likely g
    %     a_meas = a_meas * 9.806;
    % end

    label_meas = 'Phone (device z)';
else
    % Fallback: use simulated noisy channel
    t_meas = t_sol(:);
    a_meas = sprung_mass_accel(:);
    fs_meas = Sampling_Freq;
    label_meas = 'Measured (noisy)';
end

% Align clean to measured timebase for error panel/inset
a_clean_on_meas = interp1(t_sol, sprung_mass_accel_raw, t_meas, 'linear','extrap');
err_meas = a_meas - a_clean_on_meas;

% Peak events on CLEAN signal (robust)
[pksC, locsC] = findpeaks(abs(sprung_mass_accel_raw), 'MinPeakDistance', max(1,round(0.20/(1/Sampling_Freq))));
[~, ord] = sort(pksC, 'descend'); locs_top = locsC(ord(1:min(6,numel(locsC))));  % top 6 events
t_peaks = t_sol(locs_top);

% Helper: symmetric y-lims
symLim = @(x) max(1e-6, max(abs(x)))*1.15;

%% ========================================================================
% FIGURE 1 — Executive overview (Road + Heave/Deflection + Accelerations)
% =========================================================================
fig1 = figure('Name', [figName ' — Overview'],'Position',[70 60 1100 820]);
tl = tiledlayout(fig1,3,1,'TileSpacing','compact','Padding','compact');
title(tl, sprintf('%s (%.0f km/h, %dm, fs=%.0f Hz)', ...
    strrep(damage_scenario_selection,'_',' '), Vehicle_Speed, Road_Length, Sampling_Freq), ...
    'FontWeight','bold','FontSize',12);

% -------------------- Panel 1: Road profile (with A→H shade) -------------
ax1 = nexttile(tl,1); hold(ax1,'on'); box(ax1,'on'); grid(ax1,'on'); grid(ax1,'minor');
% shaded banding for Progressive_Roughness
if strcmpi(damage_scenario_selection,'progressive_roughness')
    L = spatial_road(end) - spatial_road(1);
    edges = spatial_road(1) + (0:8)/8 * L;           % 8 segments
    labelsAH = 'ABCDEFGH';
    for k = 1:8
        patch('XData',[edges(k) edges(k+1) edges(k+1) edges(k)], ...
              'YData',symLim(z_r_x*1000)*[-1 -1 1 1], ...
              'FaceColor',COL.shade,'FaceAlpha',0.30,'EdgeColor','none','Parent',ax1);
        text((edges(k)+edges(k+1))/2, 0, labelsAH(k), ...
            'HorizontalAlignment','center','VerticalAlignment','middle', ...
            'FontWeight','bold','Color',[0.25 0.25 0.25],'Parent',ax1);
    end
end
ylabel(ax1,'Elevation (mm)');
xlabel(ax1,'Distance (m)');
ylim(ax1, symLim(z_r_x*1000)*[-1 1]);
% Keep handles as you plot
h_base = [];
if exist('z_r_x_base','var') && ~isempty(z_r_x_base)
    h_base = plot(ax1, spatial_road(idx_x), z_r_x_base(idx_x)*1000, ...
                  '--', 'Color', COL.base, 'DisplayName','ISO baseline');
end
h_road = plot(ax1, spatial_road(idx_x), z_r_x(idx_x)*1000, ...
              'Color', COL.road, 'DisplayName','Road');

% Legend from actual handles
if ~isempty(h_base)
    lgd_handles = [h_base h_road]; lgd_labels = {'ISO baseline','Road'};
else
    lgd_handles = h_road;          lgd_labels = {'Road'};
end
if strcmpi(damage_scenario_selection,'progressive_roughness')
    loc = 'northeast';
else
...
legend(ax1, lgd_handles, lgd_labels, 'Location',loc, 'Box','off');

% Title: avoid string arithmetic
if strcmpi(damage_scenario_selection,'progressive_roughness')
    class_tag = 'Classes A → H';
else
    class_tag = sprintf('Class %s', road_class);
end
title(ax1, sprintf('Road Profile (%s)', class_tag));

% ---------------- Panel 2: Body heave + suspension deflection ------------
ax2 = nexttile(tl,2); hold(ax2,'on'); box(ax2,'on'); grid(ax2,'on'); grid(ax2,'minor');
h_y  = plot(ax2, t_sol(idx_t), Sprung_Mass_Disp(idx_t)*1000, 'Color', COL.heave, 'DisplayName','Body heave y_s');
h_sd = plot(ax2, t_sol(idx_t), Susp_Defl_m(idx_t)*1000,     'Color', COL.defl,  'DisplayName','Susp. deflection y_s - y_{us}');
ylabel(ax2,'Displacement (mm)');
xlabel(ax2,'Time (s)');
ylim(ax2, max([symLim(Sprung_Mass_Disp*1000) symLim(Susp_Defl_m*1000)])*[-1 1]);
legend(ax2,[h_y h_sd],'Location','northeast','Box','off');
title(ax2,'Body Heave & Suspension Deflection');

% ---------------- Panel 3: Acceleration (clean vs measured/phone) --------
ax3 = nexttile(tl,3); hold(ax3,'on'); box(ax3,'on'); grid(ax3,'on'); grid(ax3,'minor');
plot(ax3, t_sol(idx_t), sprung_mass_accel_raw(idx_t), '-', 'Color', COL.acc_clean, 'DisplayName','Clean');
if phone_exists
    % jittered alpha scatter for phone to show quantization
    p1 = plot(ax3, t_meas, a_meas, '.', 'Color', COL.acc_phone, 'MarkerSize', 3, 'DisplayName', label_meas);
else
    p1 = plot(ax3, t_sol(idx_t), sprung_mass_accel(idx_t), '-', 'Color', COL.acc_meas, 'DisplayName', label_meas);
end
% peak markers (on clean)
yl = symLim([sprung_mass_accel_raw; a_meas]);
for k = 1:numel(t_peaks)
    xline(ax3, t_peaks(k), ':', 'Color', [0.5 0.5 0.5]);
end
ylabel(ax3,'Vertical accel (m/s^2)'); xlabel(ax3,'Time (s)');
ylim(ax3, yl*[-1 1]);
% legend(ax3,'Location','northeast','Box','off');
title(ax3,'Vertical Acceleration — Clean vs Measured');

linkaxes([ax2 ax3],'x');

% % ------------------------ Inset zoom around largest event -----------------
% if ~isempty(t_peaks)
%     t0 = t_peaks(1);
% else
%     [~,i0] = max(abs(sprung_mass_accel_raw)); t0 = t_sol(i0);
% end
% win = 0.8; tL = t0 - win/2; tR = t0 + win/2;
% axesPos = ax3.Position;  % [x y w h] in normalized fig units
% inset = axes('Position', [axesPos(1)+axesPos(3)*0.60, axesPos(2)+axesPos(3)*0.07, axesPos(3)*0.35, axesPos(4)*0.50]);
% box(inset,'on'); hold(inset,'on'); grid(inset,'on');
% plot(inset, t_sol, sprung_mass_accel_raw, '-', 'Color', COL.acc_clean);
% if phone_exists
%     plot(inset, t_meas, a_meas, '.', 'Color', COL.acc_phone, 'MarkerSize', 3);
% else
%     plot(inset, t_sol, sprung_mass_accel, '-', 'Color', COL.acc_meas);
% end
% xlim(inset,[tL tR]); ylim(inset, yl*[-1 1]);
% title(inset,'Zoom'); set(inset,'FontSize',9);

% ----------------------------- Save fig 1 --------------------------------
if save_results
    try
        exportgraphics(fig1, fullfile(save_folder,'fig_overview.png'), 'Resolution', 220);
        exportgraphics(fig1, fullfile(save_folder,'fig_overview.pdf'));
    catch
        saveas(fig1, fullfile(save_folder,'fig_overview.png'));
    end
end

% %% ========================================================================
% % FIGURE 2 — Spectral & distributions (clean vs measured/phone)
% % =========================================================================
% fig2 = figure('Name', [figName ' — Spectral & Distributions'],'Position',[120 80 1100 820]);
% tl2 = tiledlayout(fig2,2,2,'TileSpacing','compact','Padding','compact');
% 
% % 1) PSD — Clean
% ax21 = nexttile(tl2,1); 
% hold(ax21,'on'); box(ax21,'on'); grid(ax21,'on'); 
% set(ax21,'XScale','log','YScale','log');   % log–log view to match ax22
% 
% % PSD params (Welch)
% win_sec  = 8;
% winN_req = max(256, 2^nextpow2(round(win_sec*Sampling_Freq)));
% winN     = min(numel(sprung_mass_accel_raw), winN_req);
% nover    = min(winN-1, round(0.5*winN));
% nfft     = max(2048, 2^nextpow2(winN));
% [PSD_c, f_c] = pwelch(sprung_mass_accel_raw, hann(winN,'periodic'), nover, nfft, Sampling_Freq);
% plot(ax21, f_c, PSD_c, 'Color', COL.acc_clean, 'DisplayName','Clean');
% xlabel(ax21,'Frequency (Hz)'); ylabel(ax21,'PSD (m^2/s^4/Hz)');
% title(ax21,'PSD — Clean');
% if numel(f_c) >= 2, xlim(ax21,[max(1e-1,f_c(2)) 80]); end
% 
% % 2) PSD Measured/Phone
% ax22 = nexttile(tl2,2); hold(ax22,'on'); box(ax22,'on'); grid(ax22,'on'); set(ax22,'XScale','log','YScale','log');
% winN_m_req = max(256, 2^nextpow2(round(win_sec*fs_meas)));
% winN_m     = min(numel(a_meas), winN_m_req);
% nover_m    = min(winN_m-1, round(0.5*winN_m));
% nfft_m     = max(2048, 2^nextpow2(winN_m));
% [PSD_m, f_m] = pwelch(a_meas, hann(winN_m,'periodic'), nover_m, nfft_m, fs_meas);
% plot(ax22, f_m, PSD_m, 'Color', phone_exists*COL.acc_phone + ~phone_exists*COL.acc_meas, 'DisplayName', label_meas);
% xlabel(ax22,'Frequency (Hz)'); ylabel(ax22,'PSD (m^2/s^4/Hz)');
% title(ax22,'PSD — Measured/Phone');
% if numel(f_m) >= 2, xlim(ax22,[max(1e-1,f_m(2)) 80]); end
% 
% % Natural frequencies markers (approx from A_chk eigen analysis if available)
% try
%     if exist('fu','var') && numel(fu)>=2
%         fmarks = fu(1:2);
%     else
%         fmarks = [sqrt(Suspension_Stiffness/Sprung_Mass), sqrt(Tire_Stiffness/Unsprung_Mass)]/(2*pi);
%     end
%     for ax = [ax21 ax22]
%         xline(ax, fmarks(1), '--', 'Body mode','LabelVerticalAlignment','bottom','Color',[0.3 0.3 0.3]);
%         xline(ax, fmarks(2), '--', 'Wheel-hop','LabelVerticalAlignment','top','Color',[0.3 0.3 0.3]);
%     end
% catch, end
% 
% % 3) Histogram + normal fit (measured/phone)
% ax23 = nexttile(tl2,3); hold(ax23,'on'); box(ax23,'on'); grid(ax23,'on');
% h = histogram(ax23, a_meas, 60, 'Normalization','pdf','FaceColor',[.85 .85 .85],'EdgeColor',[.5 .5 .5]);
% mu = mean(a_meas); sg = std(a_meas);
% xg = linspace(mu-4*sg, mu+4*sg, 500);
% plot(ax23, xg, normpdf(xg, mu, sg), '-', 'Color', phone_exists*COL.acc_phone + ~phone_exists*COL.acc_meas, 'LineWidth',1.25);
% xlabel(ax23,'a_z (m/s^2)'); ylabel(ax23,'PDF');
% title(ax23, sprintf('Distribution — %s (μ=%.3g, σ=%.3g)', label_meas, mu, sg));
% 
% % 4) Cumulative RMS vs time (clean & measured)
% ax24 = nexttile(tl2,4); hold(ax24,'on'); box(ax24,'on'); grid(ax24,'on');
% crms_clean = sqrt(cumtrapz(t_sol, sprung_mass_accel_raw.^2) ./ max(t_sol, eps));
% crms_meas  = sqrt(cumtrapz(t_meas, a_meas.^2) ./ max(t_meas, eps));
% plot(ax24, t_sol, crms_clean, 'Color', COL.acc_clean, 'DisplayName','Clean');
% plot(ax24, t_meas, crms_meas,  'Color', phone_exists*COL.acc_phone + ~phone_exists*COL.acc_meas, 'DisplayName',label_meas);
% xlabel(ax24,'Time (s)'); ylabel(ax24,'Cumulative RMS (m/s^2)');
% title(ax24,'Cumulative RMS'); legend(ax24,'Box','off','Location','best');
% 
% if save_results
%     try
%         exportgraphics(fig2, fullfile(save_folder,'fig_spectral_stats.png'), 'Resolution', 220);
%         exportgraphics(fig2, fullfile(save_folder,'fig_spectral_stats.pdf'));
%     catch
%         saveas(fig2, fullfile(save_folder,'fig_spectral_stats.png'));
%     end
% end
% 
% %% ========================================================================
% % FIGURE 3 — Phone vs Clean diagnostics (alignment, error, spectrogram)
% % =========================================================================
% fig3 = figure('Name', [figName ' — Phone Diagnostics'],'Position',[140 90 1100 880]);
% tl3 = tiledlayout(fig3,3,1,'TileSpacing','compact','Padding','compact');
% 
% % A) Aligned time histories
% ax31 = nexttile(tl3,1); hold(ax31,'on'); box(ax31,'on'); grid(ax31,'on'); grid(ax31,'minor');
% plot(ax31, t_meas, a_clean_on_meas, '-', 'Color', COL.acc_clean, 'DisplayName','Clean @ phone timebase');
% if phone_exists
%     plot(ax31, t_meas, a_meas, '.', 'Color', COL.acc_phone, 'MarkerSize', 3, 'DisplayName', label_meas);
% else
%     plot(ax31, t_meas, a_meas, '-', 'Color', COL.acc_meas, 'DisplayName', label_meas);
% end
% ylabel(ax31,'m/s^2'); xlabel(ax31,'Time (s)');
% title(ax31,'Alignment: Clean vs Phone/Measured'); legend(ax31,'Location','northeast','Box','off');
% 
% % B) Error time series + metrics
% ax32 = nexttile(tl3,2); hold(ax32,'on'); box(ax32,'on'); grid(ax32,'on'); grid(ax32,'minor');
% plot(ax32, t_meas, err_meas, '-', 'Color', COL.error, 'DisplayName','Error');
% ylabel(ax32,'m/s^2'); xlabel(ax32,'Time (s)');
% rmse_val = sqrt(mean(err_meas.^2)); mae_val = mean(abs(err_meas)); bias_val = mean(err_meas);
% title(ax32, sprintf('Error = %s - Clean  (RMSE=%.3g, MAE=%.3g, Bias=%.3g)', label_meas, rmse_val, mae_val, bias_val));
% 
% % C) Spectrogram of measured/phone (time–frequency content)
% ax33 = nexttile(tl3,3); box(ax33,'on');
% try
%     % window ~1 s, 50% overlap
%     wlen_req = max(128, 2^nextpow2(round(1*fs_meas)));
%     wlen     = min(numel(a_meas), wlen_req);
%     ov       = min(wlen-1, round(0.5*wlen));
%     nfft_s   = max(512, 2^nextpow2(wlen));
%     [S,F,T,P]= spectrogram(a_meas, hann(wlen,'periodic'), ov, nfft_s, fs_meas, 'yaxis');
%     imagesc(ax33, T, F, 10*log10(P)); axis(ax33,'xy'); colormap(ax33,'jet'); colorbar(ax33);
%     ylim(ax33,[0 100]); xlabel(ax33,'Time (s)'); ylabel(ax33,'Hz');
%     title(ax33, sprintf('Spectrogram — %s', label_meas));
% catch
%     text(0.5,0.5,'Spectrogram unavailable','Units','normalized','HorizontalAlignment','center','Parent',ax33);
% end
%
% if save_results
%     try
%         exportgraphics(fig3, fullfile(save_folder,'fig_phone_diagnostics.png'), 'Resolution', 220);
%         exportgraphics(fig3, fullfile(save_folder,'fig_phone_diagnostics.pdf'));
%     catch
%         saveas(fig3, fullfile(save_folder,'fig_phone_diagnostics.png'));
%     end
% end
...
if save_results
    ...
    % Smartphone channel
    results.smartphone = smartphone;  % t, accel_z_mps2, accel_z_unquant, settings
...
function [spatial_road, z_r_x, time_sim, z_r_t, dz_r_t, z_r_x_base] = ...
    generateRoadProfile(Road_Length, V, dt, time_road_profile, road_class, ...
                        damage_scenario, f_body_hz, f_wheelhop_hz)
    ...
    switch name
        ...
        case 'all_damages_mixed_severity'
            ...
            % Sinusoidal profile tuned to body mode frequency
            if ~isfinite(f_body_hz) || f_body_hz <= 0
                f_body_hz = 1.2;   % fallback typical body mode (Hz)
            end
            lambda_body = V / f_body_hz;   % spatial wavelength (m)

            xs = 0.20 * L;
            xe = 0.80 * L;
            m  = (spatial_road >= xs) & (spatial_road <= xe);
            xloc = spatial_road(m);

            amp = 0.010;  % 10 mm amplitude
            taper = 0.5 * (1 - cos(2*pi*(xloc - xs)/(xe - xs)));  % smooth on/off

            z_r_x(m) = z_r_x(m) + amp .* sin(2*pi*(xloc - xs)/lambda_body) .* taper;
            % baseline remains ISO-only


        case 'wheelhop_resonance_track'
            % Sinusoidal profile tuned to wheel-hop frequency
            if ~isfinite(f_wheelhop_hz) || f_wheelhop_hz <= 0
                f_wheelhop_hz = 10;  % fallback typical wheel-hop (Hz)
            end
            lambda_wh = V / f_wheelhop_hz;
            ...
            amp = 0.004;  % 4 mm amplitude
            taper = 0.5 * (1 - cos(2*pi*(xloc - xs)/(xe - xs)));

            z_r_x(m) = z_r_x(m) + amp .* sin(2*pi*(xloc - xs)/lambda_wh) .* taper;

        
        case 'cobblestone_street'
            % Dense small bumps -> cobblestone-like
            z_r_x = z_r_x * 1.2;   % slightly rougher base

            xs = 0.20 * L;
            xe = 0.70 * L;

            pitch_mean = 0.6;         % mean spacing (m)
            stones_x   = xs:pitch_mean:xe;
            jitter     = 0.15 * pitch_mean;

            for k = 1:numel(stones_x)
                xc = stones_x(k) + (2*rand-1)*jitter;
                h  = 0.005 + 0.005*rand;  % 5–10 mm
                w  = 0.30 + 0.30*rand;    % 0.3–0.6 m
                z_r_x = add_bump(spatial_road, z_r_x, xc, h, w);
            end


        case 'gravel_road'
            % Very rough gravel: higher broadband roughness + many small pits
            z_r_x = z_r_x * 3.0;   % bump ISO level up strongly

            xs = 0.10 * L;
            xe = 0.90 * L;

            % Two overlapping random pothole fields using existing fatigue proxy
            z_r_x = apply_fatigue_field(spatial_road, z_r_x, xs, xe, S.fatigue.medium, 31);
            z_r_x = apply_fatigue_field(spatial_road, z_r_x, xs, xe, S.fatigue.high,   32);

            % A couple of short corrugated bands
            lenBand = 0.15 * L;

            s1 = 0.20 * L; e1 = min(L, s1 + lenBand);
            s2 = 0.60 * L; e2 = min(L, s2 + lenBand);

            z_r_x = add_corrugation(spatial_road, z_r_x, s1, e1, S.corrug.amp.low,    S.corrug.lambda.low);
            z_r_x = add_corrugation(spatial_road, z_r_x, s2, e2, S.corrug.amp.medium, S.corrug.lambda.medium);


        case 'rail_crossing'
            % Localized high-impact rail/tram crossing
            x0  = 0.50 * L;   % crossing centre
            len = 6.0;        % local region (m)
            xs  = max(spatial_road(1), x0 - len/2);
            xe  = min(spatial_road(end), x0 + len/2);

            % Lead-in mild corrugation
            z_r_x = add_corrugation(spatial_road, z_r_x, xs, x0 - 0.5, ...
                                    S.corrug.amp.low, S.corrug.lambda.high);

            % Rails / joints as cracks
            z_r_x = add_crack(spatial_road, z_r_x, x0 - 0.6, S.crack.depth.medium, S.crack.width.medium);
            z_r_x = add_crack(spatial_road, z_r_x, x0,       S.crack.depth.high,   S.crack.width.high);
            z_r_x = add_crack(spatial_road, z_r_x, x0 + 0.6, S.crack.depth.medium, S.crack.width.medium);

            % Level mismatch as a small step
            z_r_x = add_step(spatial_road, z_r_x, x0 - 0.3, 0.020, 0.50);


        case 'random_damage_field'
            % Stochastic mix of damages over most of the length
            xs = 0.05 * L;
            xe = 0.95 * L;

            sevLevels = {'low','medium','high'};
            rng(1234);   % fixed seed for reproducibility; change if you want variety

            % Bumps
            Nb = randi([3 8]);
            for k = 1:Nb
                xc = xs + rand*(xe - xs);
                s  = sevLevels{randi(numel(sevLevels))};
                z_r_x = add_bump(spatial_road, z_r_x, xc, ...
                                 S.bump.height.(s), S.bump.width.(s));
            end
            ...
            for k = 1:Np
                xc = xs + rand*(xe - xs);
                s  = sevLevels{randi(numel(sevLevels))};
                z_r_x = add_pothole(spatial_road, z_r_x, xc, ...
                                    S.pothole.depth.(s), S.pothole.width.(s));
            end
            ...
            for k = 1:Nr
                xc = xs + rand*(xe - xs);
                s  = sevLevels{randi(numel(sevLevels))};
                z_r_x = add_rut(spatial_road, z_r_x, xc, ...
                                S.rut.depth.(s), S.rut.main_len.(s), S.rut.trans.(s));
            end

            % A bit of corrugation in one random tile
            if rand < 0.7
                span = 0.20 * (xe - xs);
                sC   = xs + rand*(xe - xs - span);
                eC   = sC + span;
                s    = sevLevels{randi(numel(sevLevels))};
                z_r_x = add_corrugation(spatial_road, z_r_x, sC, eC, ...
                                        S.corrug.amp.(s), S.corrug.lambda.(s));
            end
...
function R = rotmZYX(eul_deg)
% eul_deg = [yaw_Z, pitch_Y, roll_X] in degrees, ZYX intrinsic
    z = deg2rad(eul_deg(1)); y = deg2rad(eul_deg(2)); x = deg2rad(eul_deg(3));
    cz=cos(z); sz=sin(z); cy=cos(y); sy=sin(y); cx=cos(x); sx=sin(x);
