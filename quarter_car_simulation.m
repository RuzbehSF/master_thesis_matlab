...
noise_std_g   = 0;                        % g (accelerometer noise stdev)
...
% 'Progressive_Roughness'        (Road class A → F progression, no damage features)
...
damage_scenario_selection = 'City_Urban';
...
%% 7) Add realistic accelerometer noise
...
title(sprintf('Road Profile (Class %s): %s', road_class, strrep(damage_scenario_selection,'_',' ')));
...
function [spatial_road, z_r_x, time_sim, z_r_t, dz_r_t, z_r_x_base] = ...
    ...
    if isempty(spatial_road)
        ...
        % --- Post-synthesis rescaling to hit Gd(n0) target exactly (ISO-aligned) ---
        if numel(z_r_x) >= 32
            ...
            [Gd_hat, n_cycperm] = pwelch(z_r_x, win, nover, nfft, 1/dx, 'onesided');
            [~,k0] = min(abs(n_cycperm - ISO.n0));
            Gd_meas = max(Gd_hat(k0), eps);

            % Scale elevation by sqrt(Gd0_target / Gd_meas) so PSD hits target at n0
            scale = sqrt(Gd0_target / Gd_meas);
            z_r_x = z_r_x * scale;

            % Optional extra safety margin away from class edges:
            % margin_dB = 1.5;  z_r_x = z_r_x * 10^(-margin_dB/20);
        end
    ...
    switch name
        ...
        case 'progressive_roughness'
            ...
            % Each segment is synthesized ISO-consistently and then shifted
            % to ensure C0 continuity at the boundaries (no crossfades).
            ...
            for i = 1:numel(classes)
                ...
                % Target ISO level for this class at n0
                ISOi        = iso8608_params(classes{i}, n0);
                Gd0_target  = ISOi.center;                 % class center at n0

                % One-sided PSD and random-phase amplitude (w_iso defined earlier)
                ...
                % Gentle high-pass to suppress ultra-long drift (match global behavior)
                if Nloc >= 9
                    fs_spatial = 1/dx; nyq = fs_spatial/2;
                ...
                % Remove residual mean
                zloc = zloc - mean(zloc);

                % Rescale so pwelch at n0 matches the class center exactly
                nfft   = 2^nextpow2(min(Nloc, 2^16));
                winlen = min(max(2^nextpow2(round(Nloc/8)), 1024), Nloc);
                win    = hanning(winlen);
                nover  = round(0.5*winlen);
                [Gd_hat, n_cycperm] = pwelch(zloc, win, nover, nfft, 1/dx, 'onesided');
                [~, k0] = min(abs(n_cycperm - n0));
                Gd_meas = max(Gd_hat(k0), eps);
                zloc    = zloc * sqrt(Gd0_target / Gd_meas);

                % Enforce C0 continuity at the segment boundary
                idx = find(mloc);
                if i == 1
                    % Anchor the very first sample; global mean is removed later
                    zloc = zloc - zloc(1);
                else
                    % Shift current segment so its first sample matches previous segment's last
            ...
            % Final global conditioning to align with other scenarios
            if N >= 9
                fs_spatial = 1/dx; nyq = fs_spatial/2;
                n_c = 1/500; Wn = min(max(n_c/nyq, 1e-6), 0.99);
                [b,a] = butter(2, Wn, 'high');
