...
target_rms_m          = 0.003;  % [m] only used if calibrate_scale='match_rms'
...
%% -------------------- 4) ICA: Kurtosis Maximization (Deflation) -----------
% Paper logic: choose W to maximize non-Gaussianity (kurtosis) after
% centering + whitening. We implement a fixed-point kurtosis update:
%   w <- E{ z (w^T z)^3 } - 3 w
% with normalization and (optional) deflation orthogonalization.
...
for kComp = 1:nComp
    ...
    for it = 1:ica_max_iter
        ...
        % Kurtosis fixed-point update (pow3 nonlinearity)
        % w_new = E{ z * y^3 } - 3 w
        w = (Z * (y.^3).') / N_eff - 3*w;  % [M x 1]
...
if make_plots
    ...
    if isfield(cfg,'ground') && isfield(cfg.ground,'x') && isfield(cfg.ground,'zr')
        ...
        plot(ax10, x_common, err, '-', 'Color', C.red, 'LineWidth',1.4, 'DisplayName','Error (ICA − truth)');
        yline(ax10, 0, ':', 'Color', C.gray, 'LineWidth',1.0, 'HandleVisibility','off');
        applyTheme(ax10, useMinorGrid);
        xlabel(ax10,'Distance x [m]');
        ylabel(ax10,'Error [m]');
        title(ax10,'Reconstruction error (symmetric limits)');
